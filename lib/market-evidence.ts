...
export type ReserveData = { symbol?: string; timestamp?: string | null; sharesHeld?: string | null; circulatingSupply?: string | null };
export type OracleData = { network?: string; managedBy?: string; metadata?: Record<string, unknown> | null; name?: string; symbol?: string };
export type CorporateActionData = { eventId?: string; xstockSymbol?: string | null; caType?: string; activationDateTime?: string | null; effectiveDateUtc?: string | null; status?: string; notes?: string | null };
...
async function optional<T>(loader: () => Promise<T>): Promise<SourceResult<T>> { try { return { state: "ok", data: await loader() }; } catch { return { state: "unavailable", data: null }; } }
function solanaMint(asset: OpenStockAsset) { return asset.solanaDeployment?.address ?? null; }
function solanaUsdc(asset: OpenStockAsset) { const stablecoins = (asset.solanaDeployment?.stablecoins ?? []).map((stablecoin) => stablecoin as typeof stablecoin & { address?: string; decimals?: number }); return stablecoins.find((stablecoin) => stablecoin.currency === "USD" && stablecoin.symbol === "USDC") ?? stablecoins.find((stablecoin) => stablecoin.currency === "USD") ?? null; }
export async function getSolanaTokenDecimals(mint: string): Promise<number> { const result = await fetchJson<{ result?: { value?: { decimals?: number } } }>(SOLANA_RPC_URL, { method: "POST", body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "getTokenSupply", params: [mint] }), headers: { "Content-Type": "application/json" } }); const decimals = result.result?.value?.decimals; if (!Number.isInteger(decimals)) throw new Error("Token decimals unavailable"); return decimals as number; }
