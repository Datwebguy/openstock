...
export function PortfolioDashboard() {
  ...
  const [message, setMessage] = useState("Connect a wallet to see its balances.");
  ...
  const loadPortfolio = useCallback(async () => {
    if (!wallet) return;
    setLoading(true);
    setMessage("Reading balances and current prices…");
    try {
      ...
      setMessage(payload.valueComplete ? "Updated just now." : "Balances are live; some prices are still being refreshed.");
    } catch (error) {
  ...
  return <section className={styles.shell} aria-live="polite">
    {!wallet ? <div className={styles.connectCard}>
      ...
      <div><div className="eyebrow">Read-only portfolio</div><h2>Your holdings, in one calm view.</h2><p>Connect your wallet to read balances and current market value. OpenStock never creates or controls wallets.</p><button className="button button--gradient" type="button" onClick={connect}>Connect wallet</button><p className={styles.status}>{message}</p></div>
    </div> : <>
      ...
      <div className={styles.contentGrid}>
        <section className={styles.holdingsCard}><div className={styles.cardHeading}><div><div className="eyebrow">Assets in this wallet</div><h3>Holdings</h3></div><span>{portfolio?.holdings.length ?? 0} positions</span></div>
          {!portfolio ? <div className={styles.loadingList}>{[1, 2, 3].map((item) => <div className={styles.skeleton} key={item} />)}</div> : portfolio.holdings.length === 0 ? <div className={styles.empty}><strong>No xStocks found yet.</strong><p>When this wallet holds a supported xStock, it will appear here with its live market value.</p><Link className="button button--light" href="/app">Browse stocks</Link></div> : <div className={styles.holdingList}>{portfolio.holdings.map((holding) => { const share = allocationTotal > 0 && holding.valueUsd !== null ? (holding.valueUsd / allocationTotal) * 100 : null; return <div className={styles.holding} key={holding.symbol}><StockLogo symbol={holding.symbol} logo={holding.logo ?? undefined} size={44} /><div className={styles.holdingName}><strong>{holding.name}</strong><span>{holding.symbol} · {number(holding.shares)} shares</span></div><div className={styles.holdingValue}><strong>{money(holding.valueUsd)}</strong><span>{holding.priceUsd === null ? "Price refreshing" : money(holding.priceUsd) + " per share"}</span></div><div className={styles.allocation}><span>{share === null ? "—" : share.toFixed(1) + "%"}</span><i><b style={{ width: Math.min(100, share ?? 0) + "%" }} /></i></div></div>; })}</div>}
        </section>
        <aside className={styles.contextCard}><div className="eyebrow">What this view knows</div><h3>Balances first. Performance when earned.</h3><p>Market value comes from live wallet balances and public issuer prices. Historical change appears after OpenStock has enough snapshots to calculate it.</p><div className={styles.contextRows}><div><span>Wallet</span><strong>{shortWallet(wallet)}</strong></div><div><span>Last checked</span><strong>{portfolio ? new Date(portfolio.generatedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "Refreshing"}</strong></div><div><span>Snapshots</span><strong>{portfolio ? portfolio.performance.snapshots : 0}</strong></div><div><span>Data mode</span><strong>Read-only</strong></div></div><Link href="/app/analytics" className={styles.textLink}>Open market analytics →</Link></aside>
      </div>
    </>}
  </section>;
}
