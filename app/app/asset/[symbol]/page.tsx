...
export default async function AssetPage({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  const facts = [
    ...
    { label: "Liquidity", value: pool ? "$" + number(pool.tvl, 0) : "PENDING" },
  ...
  return <main className="page">
    ...
    <div className="app-page container asset-workspace">
      ...
      <header className="asset-detail-header">
        ...
        <div className="asset-status-row"><span className={halted ? "status-chip status-chip--warn" : "status-chip status-chip--good"}>{venueStatus}</span><span>{issuerStatus}</span><span>{asset.trading?.openNow ? "OPEN" : marketClosed ? "HALT" : "PENDING"}</span></div>
      </header>
