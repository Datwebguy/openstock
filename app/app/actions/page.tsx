...
export default function CorporateActionsPage() {
  return <main className="page"><AppNav ctaHref="/app" ctaLabel="Market" /><div className="container app-page"><Link href="/app" className="back-link">← Market</Link><header className="app-market-header history-header"><div><h1>Events</h1><p className="workspace-description">Stay close to corporate actions and the share adjustments that affect your holdings.</p></div></header><CorporateActionsWorkspace symbols={CURATED_SYMBOLS.map((symbol) => ({ symbol, name: names[symbol] ?? symbol }))} /></div><AppFooterNav /></main>;
}
