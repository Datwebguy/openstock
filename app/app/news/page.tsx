...
export default function NewsPage() {
  return <main className="page"><AppNav ctaHref="/app" ctaLabel="Market" /><div className="container app-page news-page"><Link href="/app" className="back-link">← Market</Link><header className="news-header"><h1>News</h1><p className="workspace-description">Real-time market headlines &amp; issuer updates.</p></header><MarketNewsFeed symbols={CURATED_SYMBOLS.map((symbol) => ({ symbol, name: names[symbol] ?? symbol }))} /></div><AppFooterNav /></main>;
}
