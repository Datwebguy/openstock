...
function stablecoin(asset: Awaited<ReturnType<typeof getHydratedAsset>>) {
  const stablecoins = (asset.solanaDeployment?.stablecoins ?? []) as Array<{
    currency?: string;
    symbol?: string;
    address?: string;
    decimals?: number;
  ...
  return (
    stablecoins.find((coin) => coin.currency === "USD" && coin.symbol === "USDC") ??
    stablecoins.find((coin) => coin.currency === "USD") ?? { address: USDC_MINT, decimals: 6 }
  );
}
...
export async function POST(request: Request) {
  if (!process.env.JUPITER_API_KEY) {
    return NextResponse.json(
      { error: "Live routing is not enabled. Review this order as a paper trade." },
  ...
  try {
    ...
    return NextResponse.json({ error: "Enter a valid order." }, { status: 400 });
  }
  const symbol = body.symbol?.trim(),
    side = body.side,
    shares = body.shares,
    wallet = body.wallet?.trim();
  if (!symbol || (side !== "buy" && side !== "sell") || !Number.isFinite(shares) || Number(shares) <= 0 || !wallet) {
    return NextResponse.json({ error: "Enter a stock, share amount, and wallet before continuing." }, { status: 400 });
  }
  try {
    new PublicKey(wallet);
  } catch {
    return NextResponse.json({ error: "That wallet address is not valid. Reconnect your wallet and try again." }, { status: 400 });
  }
  try {
    ...
    if (
      ...
      return NextResponse.json({ error: "No live issuer or onchain price is available for this order." }, { status: 409 });
    }
    const coin = stablecoin(asset);
    const stableMint = coin.address ?? USDC_MINT;
    const stableDecimals =
      typeof coin.decimals === "number" && Number.isInteger(coin.decimals) && coin.decimals >= 0 && coin.decimals <= 18
        ? coin.decimals
        : USDC_DECIMALS;
    const inputMint = side === "buy" ? stableMint : stockMint;
    const outputMint = side === "buy" ? stockMint : stableMint;
    const sellConversion = side === "sell" ? uiToRaw(Number(shares), multiplier, decimals) : null;
    if (side === "sell" && !sellConversion) {
      return NextResponse.json({ error: "The share amount could not be converted safely." }, { status: 400 });
    }
    const inputAmount =
      side === "buy"
        ? String(Math.max(1, Math.round(Number(shares) * price * 10 ** stableDecimals)))
        : sellConversion!.rawAmount;
    ...
    // Jupiter order block height ≈ ~2s/slot; treat as soft expiry hint for the client.
    const expiresAt = order.lastValidBlockHeight
    ...
    return NextResponse.json(
      {
        ...
        outputMint,
        inputAmount,
        uiAmount: Number(shares),
        rawAmount: side === "sell" ? sellConversion!.rawAmount : null,
    ...
    return NextResponse.json(
      {
        error:
          status === 404 ? "Stock not found." : "We could not prepare a live route. Try refreshing the market context.",
      },
